					       ...
					       const DL_ISO8583_MSG     *iMsg )
{
	...
	for ( i=0 ; i<(iHandler->fieldItems) ; i++ )
	{
		if ( NULL != iMsg->field[i].ptr ) /* present */
		{
			...
			fprintf(iOutFile,"[%03d][type:%d,len:%hu,varlen:%d] ",
					(int)i,fieldDef->fieldType,fieldDef->len,fieldDef->varLen);
			fprintf(iOutFile,"%d:%s%s",iMsg->field[i].len,iMsg->field[i].ptr,tmpEOL);
		}
